...
Defaults to `%LocalAppData%/octobuild` on Windows, `~/.cache/octobuild` on Linux and `~/Library/Caches/octobuild` on macOS.
`OCTOBUILD_CACHE_LIMIT_MB`:: specifies octobuild disk cache size limit in megabytes.
Defaults to 64GB.
`OCTOBUILD_PROCESS_LIMIT`:: specifies max number of concurrent processes octobuild will spawn.
